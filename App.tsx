...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const initSession = async () => {
      ...
      if (session) {
        ...
        // If there was an item being processed, we can restore it to the UI or queue it
        // The prompt says: "revert to 'queued' (so the team can re-run it)"
        // We can just show a toast saying we recovered it, or maybe pre-fill the text area if it was text.
        ...
        if (session.restoredProcessingItem) {
             ...
             // Optionally we could set this text into the paste area or similar, 
             // but for now just letting them know is good.
        }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      saveSession(dailyQueue, activeView, intakeMode, processingItem);
    }, 500);

    return () => clearTimeout(timer);
  }, [dailyQueue, activeView, intakeMode, processingItem, isRestored]);
    ...
    } catch (err)
 {
  ...
  const handleExportThisPoXlsx = useCallback(() => {
    ...
    const extractionRows = productionOrderToExportRows(extractedData, fileName);
    const tintingItemsForOrder = filterTintingItemsFromOrder(extractedData);
    const tintingExportRows = tintingListToExportRows(tintingItemsForOrder);
    ...
    if (tintingExportRows.length > 0) {
        sheets.push({ sheetName: 'Tinting', rows: tintingExportRows });
    }
    
    exportToXlsx(sheets, `PO-${extractedData.order.order_number || 'export'}.xlsx`);
  }, [extractedData, fileName]);
  ...
  const handleExportXlsx = useCallback(async () => {
    try {
      ...
      // 3. Align data
      // We convert queueRows (ExportRow[]) to Record<string, any>[] implicitly
            ...
            <nav className="-mb-px flex space-x-6" aria-label="Tabs">
                ...
                >
                    ...
                    <span className={`ml-2 text-xs font-semibold px-2 py-0.5 rounded-full ${
                      activeView === 'tinting' ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-200 text-slate-600'
                    }`}>
                      {tintingItems.length}
                    </span>
                </button>
                  ...
                  <div className="flex items-center justify-between gap-3 mb-4 pb-4 border-b border-amber-300">
                    ...
                    {extractionSource && (
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        extractionSource === 'PDF' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                      }`}>
